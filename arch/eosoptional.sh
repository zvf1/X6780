#!/bin/bash
#
# eosoptional.sh  (Arch / EndeavourOS)
#
# Combined installer + boot script (formerly install-lzscript.sh + lzScript.sh).
#
# Run manually (e.g. via curl | bash) to install:
#   1. Verifies intel-undervolt is installed.
#   2. Sets CPU / CPU Cache undervolt offsets to -155 and enable=yes in
#      /etc/intel-undervolt.conf.
#   3. Installs this script to /usr/local/sbin/eosoptional.sh.
#   4. Registers + enables eosoptional.service to run it on every boot.
...
# View logs with: journalctl -u eosoptional.service
#
...
run_boot_tasks() {
    ...
    if command -v intel-undervolt &>/dev/null; then
        ...
        echo "Error: intel-undervolt not found (expected to be installed via pacman)." >&2
    fi
...
if ! command -v intel-undervolt &>/dev/null; then
    echo "Error: intel-undervolt not found. Install it first:" >&2
    echo "  sudo pacman -S intel-undervolt" >&2
    exit 1
fi
...
echo "==> Installing eosoptional.sh to ${SCRIPT_PATH}..."
if [[ -f "$0" && "$0" != "bash" && "$0" != "-bash" && -r "$0" ]]; then
    sudo install -Dm755 "$0" "$SCRIPT_PATH" 2>/dev/null || sudo curl -fsSL "$RAW_BASE/eosoptional.sh" -o "$SCRIPT_PATH"
else
    sudo curl -fsSL "$RAW_BASE/eosoptional.sh" -o "$SCRIPT_PATH"
fi
sudo chmod 755 "$SCRIPT_PATH"

echo "==> Installing eosoptional.service..."
...
sudo systemctl enable eosoptional.service

echo "==> Done. eosoptional.service is enabled and will run on next boot."
echo "    Run it now with: sudo systemctl start eosoptional.service"
echo "    View logs with:  journalctl -u eosoptional.service"
